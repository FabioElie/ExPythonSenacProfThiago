...
vencedor = max(candidatos, key=candidatos.get)  # type: ignore
...
print(
    f"José: teve  o total de {votos_jose} votos, percentual de votos {(votos_jose /total_votos) * 100:.2f}%"
)
print(
    f"João: teve  o total de {votos_joao} votos, percentual de votos {(votos_joao /total_votos) * 100:.2f}%"
)
print(
    f"Maria: teve  o total de {votos_maria} votos, percentual de votos {(votos_maria /total_votos) * 100:.2f}%"
)
print(
    f"Ana: teve  o total de {votos_ana} votos, percentual de votos {(votos_ana /total_votos) * 100:.2f}%"
)
print(f"Votos nulos: {votos_nulos}, percentual de votos nulos: {perc_nulos:.2f}%")
print(
    f"Votos em branco: {votos_brancos}, percentual de votos em branco: {perc_brancos:.2f}%"
)
